 ...
 */
 async function addUser(username, password, firstName, lastName) {
  ...
  if (!validate.isValidUserName(username)) {
   throw new InvalidInputError("Invalid username. Must only contain numbers and/or letters.");
 }
 ...
 */
 async function getAllUsers() {
  ...
  if (result[0].length == 0) {
      ...
      throw new InvalidInputError("No users exists");
  }
...
async function UpdateUserPassword(originalUsername, updatePassword) {
   ...
   if (!validate.isValidUserName(originalUsername)) {
     throw new InvalidInputError("Invalid username. Must only contain numbers and/or letters.");
   }
 ...
 */
 async function DeleteUser(originalUsername) {
  ...
  try {
    ...
    logger.error(error);
    if (error instanceof InvalidInputError) {
      throw new InvalidInputError("Username does not exist.");
    }
    throw new DBConnectionError();
  }
}
